  ...
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (!data) {
        return res
          .status(404)
          .json({ error: "No note with given ID." });
      }
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!title && !content) {
      return res
        .status(400)
        .json({ error: "No parameters to update were specified." });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      const updateDoc = { $set: (title && content) ? {
        "title": title,
        "content": content
      } : title ? {"title": title} : {"content": content}}

      await collection.updateOne(data, updateDoc)
